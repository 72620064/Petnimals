...
public class ExportarExcel {
    public void productos(String ruta, List<Producto> productos) {
        try {
            ...
            row0.createCell(2).setCellValue("PRECIO");
            row0.createCell(3).setCellValue("STOCK");
            
            for (Producto p : productos) {
                var row = sheet.createRow(fila++);
                row.createCell(0).setCellValue(p.getIdProducto());
                row.createCell(1).setCellValue(p.getNombre());
                row.createCell(2).setCellValue(p.getPrecio() != null ? p.getPrecio().doubleValue() : 0.0);
    ...
    public void usuarios(String ruta, List<Usuario> usuarios) {
        try {
            ...
            for (Usuario p : usuarios) {
                ...
                row.createCell(1).setCellValue(p.getCorreo());
                row.createCell(1).setCellValue(p.getTelefono());
            }
